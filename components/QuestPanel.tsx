...
import { ReactDOM, Tooltip, useEffect, useState } from "@webpack/common";

import { getQuestRuntime, isPanelOpen, QuestRuntime, setPanelOpen, subscribeQuestState } from "../questState";
...
/** Rendered once, into the body, so the title bar cannot clip it. */
export function QuestPanelHost() {
    const [, forceUpdate] = useState(0);

    useEffect(() => subscribeQuestState(() => forceUpdate(n => n + 1)), []);

    if (!isPanelOpen()) return null;
    return ReactDOM.createPortal(<QuestPanel />, document.body);
}
